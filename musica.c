...
No* remover_musica(No *lista, const char *titulo) {
    if (lista == NULL) {
        printf("Lista vazia. Nenhuma musica para remover.\n");
        return NULL;
    }
    ...
    if (atual != NULL) {
        ...
        printf("Musica \'%s\' nao encontrada na lista.\n", titulo);
    }
    return lista;
}
...
void salvar_musicas_para_arquivo(No *lista, const char *nome_arquivo) {
    ...
    fclose(arquivo);
    printf("Musicas salvas em %s com sucesso.\n", nome_arquivo);
}
...
No* carregar_musicas_de_arquivo(const char *nome_arquivo) {
    ...
    fclose(arquivo);
    printf("Musicas carregadas de %s com sucesso.\n", nome_arquivo);
    return lista;
}
